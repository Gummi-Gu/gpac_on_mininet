...
# ====================== 客户端智能体 ======================
class DQNClient:
    def __init__(self, client_id, gamma, init_model_path=None):
...
class LocalEnvSimulator:
    ...
    def get_state(self):
        ...
        for _ in range(N):
            ...
            band_list = []
            for client_id, stats in temp.items():
                band = stats['size']/stats['time']*1000  # 获取客户端的 QoE 值
                if band is not None:
        ...
        avg_size = {k: predict_next(v) if len(v) == N else None for k, v in avg_size.items()}
...
def client_training_loop(client_id, num_episodes, init_model):
    # 初始化
    client = DQNClient(client_id, 0.5, init_model)
...
if __name__ == "__main__":
    ...
    init_model = sys.argv[3] if len(sys.argv) > 3 else None  # 新增第三个参数
